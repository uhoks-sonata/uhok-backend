...
FastAPI 기반으로 구성되어 있으며, 사용자, 홈쇼핑, 콕, 주문, 레시피 추천, 로깅 등의 핵심 기능을 제공합니다.
...
- **라이브 스트리밍**: 실시간 방송 스트림 URL/메타데이터(JSON) 조회 및 관리
...
- **주문 조회**: 주문 및 배송 정보 조회
- **동시성 제어**: 장바구니 중복 주문 방지(`SELECT FOR UPDATE`), 중복 웹훅 수신 시 상태 충돌 방지(`SERIALIZABLE`)
...
- **사용자 행동 로그**: 사용자 활동 추적 및 분석을 위한 로그 수집
...
#### 1. 계층별 역할 분리
- **Router**: HTTP 요청/응답 처리, 파라미터 검증, 의존성 주입, 트랜잭션 커밋 (`db.commit()`)
- **CRUD**: 비즈니스 로직, 데이터베이스 처리 (`db.add()`, `db.flush()`) — 트랜잭션 경계는 담당하지 않음. 동시성 제어가 필요한 경우 `SELECT FOR UPDATE` 및 `SERIALIZABLE` 격리 수준 적용 ([참고](docs/db_concurrency_control.md))
- **Model**: 데이터베이스 스키마 정의
...
#### 개선사항 성능평가서
[백엔드 개선사항 성능평가서](docs/백엔드_개선사항_성능평가서.pdf)

#### 결제통신 로직 변경사항
[결제통신로직 변경사항](docs/결제통신로직_변경사항.pdf)
...
- ~~**사용자 행동 분석**: 사용자 이벤트 로깅을 통해 수집된 데이터는 개인화 개선을 위해 분석 시스템으로 전달됩니다.~~
...
- **XSS 방지**: 입력 데이터 유효성 검증 (Pydantic 스키마)을 통해 1차적으로 방어하며, 출력 시 프론트엔드 또는 템플릿 엔진에서 적절한 이스케이핑을 통해 최종적으로 방어합니다.
- **CSRF 보호**: CORS 설정을 통해 허용된 도메인에서의 요청만 허용하여 크로스 사이트 요청을 방지합니다. 직접적인 CSRF 토큰 방식의 보호는 구현되어 있지 않습니다.
...
- **로그 시스템 아키텍처 변경**: 현재 PostgreSQL에 저장되는 사용자 이벤트/행동 로그를 별도의 OLAP 저장소(예: ClickHouse, BigQuery)로 이전할 예정입니다. 이를 통해 `uhok-backend`는 로그 데이터 조회 부담을 줄이고, 분석 팀은 OLAP 환경에서 더 효율적으로 데이터를 활용할 수 있게 됩니다. 이 변경으로 인해 백엔드의 PostgreSQL 로그 데이터 직접 접근 기능은 제거됩니다.
